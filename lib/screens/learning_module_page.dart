...
import 'database_service.dart'; // make sure this path is correct
...
class _LearningModulePageState extends State<LearningModulePage> {
  ...
  Future<void> _showCompletionDialog() async {
    int correctCount = 0;
    // For simplicity assuming answered correctly for demo
    correctCount = (questions.length * 0.8).toInt(); // 80% correct
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text(
          'Quiz Completed!',
          style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF21C573)),
        ),
        content: Column(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            bool hasBadges = badges.values.any((b) => b != null);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.moduleType[0].toUpperCase()}${widget.moduleType.substring(1)} Module'),
      ...
      children: [
        ...
        ...(question['answers'] as List<Map<String, Object>>).map((answer) {
          return Container(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF1791B6),
